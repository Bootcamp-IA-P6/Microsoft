...
# MAGIC %md
# MAGIC # Poll arrives → bronze → silver_arrives → gold ETA/freq (contract v4.3.1)
# MAGIC Does **not** update Gold `alert_*` — that is `nb_alerts_silver_gold`.
# MAGIC Freq: visit first-seen observations at stop×line×direction ([ADR-038](../docs/adr/ADR-038-observed-headway-passages-are-bus-visit-first-seen-at-stop.md)).
...
def compute_freq_by_line(spark, *, freq_min: int) -> dict[str, dict]:
...
def run_transform(spark, *, stale_after_sec: int, bronze_table: str, incremental: bool, freq_min_samples: int) -> None:
    ...
    freq_by_line = compute_freq_by_line(spark, freq_min=freq_min)
