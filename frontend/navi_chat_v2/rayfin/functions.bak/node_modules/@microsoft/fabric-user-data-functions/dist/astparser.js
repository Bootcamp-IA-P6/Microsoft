...
class TypeScriptProjectParser {
    ...
    constructor(rootPath = "./src/functions", customFabricTypes) {
     ...
     */
    scanProject() {
        const tsFiles = this.discoverTypeScriptFiles();
     ...
     */
    extractFabricUdfFunctionsFromFile(parsedFile) {
        const udfFunctions = [];
        const visit = (node) => {
     ...
     */
    extractFabricUdfFunctionInfo(node, filePath) {
        ...
        const delegateParameters = delegateArg.parameters.map((param, index) => {
     ...
     */
    isFabricParameterType(type, name) {
        // Check by type first
     ...
     */
    validateParameters(delegateParameters, genericType) {
        const validation = {
            hasErrors: false,
            missingParameters: [],
            extraParameters: [],
     ...
     */
    resolveTypeStructure(typeName, contextFilePath) {
     ...
     */
    resolveTypeFromImports(typeName, contextFilePath) {
        const contextParsedFile = this.parsedFiles.get(contextFilePath);
        if (!contextParsedFile)
            return undefined;
     ...
     */
    resolveImportPath(moduleSpecifier, contextFilePath) {
     ...
     */
    resolveTypeFromWorkspace(typeName) {
     ...
     */
    findTypeScriptFilesInProject(projectRoot) {
        const files = [];
        const maxFiles = 50; // Limit to prevent performance issues
        const scanDirectory = (dir, depth = 0) => {
            if (files.length >= maxFiles || depth > 3)
     ...
     */
    parseAdditionalTypeFile(filePath) {
        if (!this.program)
            return;
        try {
     ...
     */
    extractTypeStructureFromFile(typeName, parsedFile) {
        let foundStructure;
        const visit = (node) => {
     ...
     */
    parseTypeNode(typeNode, typeName) {
     ...
     */
    parseTypeLiteral(node, typeName) {
        const properties = [];
        for (const member of node.members) {
            if (ts.isPropertySignature(member) && member.name) {
                const name = member.name.getText();
     ...
     */
    extractFunctionsFromFile(parsedFile) {
        const functions = [];
        const visit = (node) => {
     ...
     */
    extractFunctionInfo(node, filePath) {
     ...
     */
    extractImportsFromFile(parsedFile) {
        const imports = [];
        const visit = (node) => {
     ...
     */
    extractImportInfo(node, filePath) {
     ...
     */
    findNodesByType(syntaxKind, predicate) {
        const results = [];
        for (const parsedFile of this.parsedFiles.values()) {
            const visit = (node) => {
...
class ASTNodeVisitor {
    static visitAllNodes(sourceFile, callback, depth = 0) {
    ...
    static findNodeAtPosition(sourceFile, position) {
        function find(node) {
...
export { TypeScriptProjectParser, ASTNodeVisitor, initializeParser };
