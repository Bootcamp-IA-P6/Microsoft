...
async function callDataAgent(question: string, token: string): Promise<string> {
...
function mockAnswer(question: string, _language: string): { answerText: string } {
...
udf.func(
  'chat',
  async (question: string, language: string): Promise<{ answerText: string }> => {
    if (isLocalDev()) {
